    ...
    while (!isValidImage) {
        ...
        try {
            if (image.loadNewImage(imgname)) {
                isValidImage = true; // Set flag to true to exit the loop
...
string saveImageWithExtension() {
    ...
    while (!isValidExtension) {
        cout << "Please enter the filename to store the flipped image, and specify extension (.jpg, .bmp, .png, .tga): ";
        cin >> newname;
        ...
        if (newname.length() >= 4 && (newname.substr(newname.length() - 4) == ".jpg" ||
            ...
            cout << "Invalid filename extension. Please use ( .jpg, .bmp, .png, or .tga. )!" << endl;
        }
    }
    return newname;
...
// filter 8 Crop Images
 Image Crop_Images(Image& image)
 {
     ...
     {
         cerr << "Error: Crop area exceeds the bounds of the original image." << endl;
         cout << "==========================================================" << endl;
